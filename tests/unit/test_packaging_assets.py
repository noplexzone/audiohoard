...
def test_dockerfile_version_and_healthcheck_match_current_runtime_contract() -> None:
    ...
    assert 'org.opencontainers.image.version="0.24.0"' in dockerfile
...
def test_review_deck_alignment_switches_one_player_at_equivalent_times() -> None:
    ...
    stylesheet = Path("app/static/css/pages.css").read_text(encoding="utf-8")
    assert 'data-alignment-state="defaulted"' in stylesheet
    assert 'data-alignment-state="estimated"' not in stylesheet
